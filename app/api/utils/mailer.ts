...
export async function sendAdminMail(order: any) {
  ...
  try {
    ...
    console.error('❌ Échec critique de sendAdminMail. Détails de l\'erreur :', error.message)
    // Loggez l'objet d'erreur complet si possible pour voir le code ('ETIMEDOUT', 'EAUTH')
    console.error(error)
  }
